...
int agregarPelicula(char fileName[], EMovie *movie)
{
    ...
    fMovies=fopen(fileName, "ab");
    if(fMovies==NULL)
    {
        printf("Error al abrir el archivo %s", fileName);
        ret=-1;
    }
    cant=fwrite(movie, sizeof(EMovie), 1, fMovies);
    if(cant!=1)
    {
        printf("No se pudo agregar la pelicula en el archivo %s", fileName);
        ret= -1;
    }else
    {
        printf("La pelicula se agrego exitosamente\n");
    }
